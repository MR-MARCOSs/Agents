...
ARQUIVO_ENTRADA = "diretriz_limpa.txt"
ARQUIVO_SAIDA = "chunks_para_rag.json"
SUMARIO_TEXTO = """
Introdução ............................................................................................795
...
def extrair_secoes(conteudo, lista_titulos):
    """Localiza o início e fim de cada seção no texto."""
    posicoes = []
    for titulo in lista_titulos:
        pattern = re.escape(titulo).replace(r'\ ', r'\s+')
        match = re.search(pattern, conteudo, re.IGNORECASE)
        if match:
            posicoes.append({'titulo': titulo, 'inicio': match.start(), 'fim_header': match.end()})
    
    secoes_com_texto = []
    for i in range(len(posicoes)):
        titulo_atual = posicoes[i]['titulo']
        inicio_texto = posicoes[i]['fim_header']
        fim_texto = posicoes[i+1]['inicio'] if i+1 < len(posicoes) else len(conteudo)
        
        texto_secao = conteudo[inicio_texto:fim_texto].strip()
        secoes_com_texto.append({'titulo': titulo_atual, 'texto': texto_secao})
    
    return secoes_com_texto

def criar_chunks(secoes):
    """Divide seções longas em chunks menores injetando metadados."""
    ...
    documentos_finais = []
    for secao in secoes:
        if len(secao['texto']) < 5: continue 
        pedacos = splitter.split_text(secao['texto'])
        for i, pedaco in enumerate(pedacos):
            
            conteudo_estruturado = f"CAPÍTULO: {secao['titulo']}\n\nCONTEÚDO: {pedaco}"
            
            documentos_finais.append({
                "text": conteudo_estruturado,
                "metadata": {
                    "secao": secao['titulo'],
                    "chunk_id": i,
                    "tamanho": len(pedaco)
...
titulos_limpos = []
for linha in SUMARIO_TEXTO.strip().split('\n'):
    linha_limpa = re.sub(r'\s*\.+\s*\d+$', '', linha).strip()
    if linha_limpa: titulos_limpos.append(linha_limpa)

try:
    with open(ARQUIVO_ENTRADA, "r", encoding="utf-8") as f:
        texto_completo = f.read()
except FileNotFoundError:
    print(f"Erro: Crie o arquivo {ARQUIVO_ENTRADA} com o conteúdo do PDF.")
    exit()


print("Processando seções...")
secoes_extraidas = extrair_secoes(texto_completo, titulos_limpos)
print(f"Foram identificadas {len(secoes_extraidas)} seções.")
print("Criando chunks para RAG...")
chunks_finais = criar_chunks(secoes_extraidas)

with open(ARQUIVO_SAIDA, "w", encoding="utf-8") as f:
    json.dump(chunks_finais, f, ensure_ascii=False, indent=4)

print(f"Sucesso! Gerados {len(chunks_finais)} chunks no arquivo {ARQUIVO_SAIDA}")